...
static constexpr std::integral_constant<bool, false>
__check_compatible_extents(std::integral_constant<bool, false>, std::integer_sequence<size_t, Extents...>,
...
__check_compatible_extents(std::integral_constant<bool, true>, std::integer_sequence<size_t, Extents...>,
                           std::integer_sequence<size_t, OtherExtents...>) noexcept {
    return {};
}
...
public:
    ...
    MDSPAN_TEMPLATE_REQUIRES(
        ...
        /* requires */ (_MDSPAN_FOLD_AND(_MDSPAN_TRAIT(is_convertible, OtherIndexTypes, index_type) /* && ... */)
    ...
    constexpr extents(const array<OtherIndexType, N> &exts) noexcept : m_vals(std::move(exts)) {}
    ...
    MDSPAN_INLINE_FUNCTION
    MDSPAN_CONDITIONAL_EXPLICIT(N != m_rank_dynamic)
    constexpr extents(const span<OtherIndexType, N> &exts) noexcept : m_vals(std::move(exts)) {}
#endif
...
private:
    ...
    MDSPAN_INLINE_FUNCTION
    vals_t __construct_vals_from_extents(std::integral_constant<size_t, DynCount>, std::integral_constant<size_t, R>,
    ...
    MDSPAN_INLINE_FUNCTION
    vals_t __construct_vals_from_extents(std::integral_constant<size_t, DynCount>, std::integral_constant<size_t, R>,
                                         const OtherExtents &, DynamicValues... dynamic_values) noexcept {
...
public:
    ...
    constexpr extents(const extents<OtherIndexType, OtherExtents...> &other) noexcept
        : m_vals(__construct_vals_from_extents(std::integral_constant<size_t, 0>(), std::integral_constant<size_t, 0>(),
                                               other)) {}
...
static
#endif
    constexpr bool __is_extents_v
    = __is_extents<T>::value;
