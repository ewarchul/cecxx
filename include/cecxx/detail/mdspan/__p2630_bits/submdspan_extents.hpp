...
MDSPAN_INLINE_FUNCTION constexpr auto inv_map_rank(integral_constant<size_t, Counter>, index_sequence<MapIdxs...>,
    ...
    using next_idx_seq_t = conditional_t<is_convertible_v<Slice, size_t>, index_sequence<MapIdxs...>,
...
MDSPAN_INLINE_FUNCTION
constexpr integral_constant<size_t, 0> first_of(const experimental::full_extent_t &) {
...
MDSPAN_TEMPLATE_REQUIRES(size_t k, class Extents, class Integral,
                         /* requires */ (is_convertible_v<Integral, size_t>))
MDSPAN_INLINE_FUNCTION
constexpr Integral last_of(integral_constant<size_t, k>, const Extents &, const Integral &i) {
    return i;
}
...
MDSPAN_TEMPLATE_REQUIRES(size_t k, class Extents, class Slice,
                         /* requires */ (is_convertible_v<Slice, tuple<size_t, size_t>>))
MDSPAN_INLINE_FUNCTION
constexpr auto last_of(integral_constant<size_t, k>, const Extents &, const Slice &i) {
    return get<1>(i);
}
...
MDSPAN_INLINE_FUNCTION constexpr auto last_of(integral_constant<size_t, k>, const Extents &ext,
...
struct extents_constructor {
    ...
    constexpr static auto next_extent(const Extents &ext, const Slice &sl, SlicesAndExtents... slices_and_extents) {
        ...
        return next_t::next_extent(ext, slices_and_extents...,
                                   index_t(last_of(integral_constant<size_t, Extents::rank() - K>(), ext, sl))
                                       - index_t(first_of(sl)));
    }
