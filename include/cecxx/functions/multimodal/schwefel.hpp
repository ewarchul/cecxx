...
constexpr auto schwefel(std::span<const f64> input) -> f64 {
  ...
  for (auto i = 0u; i < nrow; i++) {
    if ((input[i] + shift) > 500) {
      output -=
          (500.0 - std::fmod((input[i] + shift), 500)) *
      ...
      output -=
          (-500.0 + std::fmod(std::fabs((input[i]) + shift), 500)) *
          std::sin(std::pow(
              500.0 - std::fmod(std::fabs((input[i] + shift)), 500), 0.5));
      tmp = ((input[i] + shift) + 500.0) / 100;
      output += tmp * tmp / static_cast<f64>(nrow);
    } else {
      output -= (input[i] + shift) *
                std::sin(std::pow(std::fabs((input[i] + shift)), 0.5));
    }
  }
