...
auto invoke_when_any(std::shared_ptr<Executor> executor,
                     ...
                     std::invocable<T> auto &&callable) -> concurrencpp::result<void> {
  while (not results.empty()) {
    auto when_any_result =
        co_await concurrencpp::when_any(executor, results.begin(), results.end());
    auto finished_task = std::move(when_any_result.results[when_any_result.index]);
