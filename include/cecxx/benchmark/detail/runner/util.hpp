...
auto create_benchmark_tasks(benchmark_specification)
    -> std::vector<benchmark_task> {
  return {};
}
...
struct run_problem_trials_fn {
  auto operator()(auto &&optimizer, benchmark_task task) const -> trial_result {
    ...
    for (const auto trial_num : std::ranges::views::iota(0, task.trials_num)) {
      auto result = optimizer(task.evaluation_function,
                              make_runif_vector(task.dimension));
