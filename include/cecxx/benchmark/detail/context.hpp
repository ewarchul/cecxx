...
template <typename Number>
using geom_transform_dim_fn_grid =
    std::unordered_map<dimension, geom_transfrom_fn_grid<Number>>;
...
public:
  ...
  auto shift(const usize fn, const usize dim) const {
    return shift_.at(dim).at(fn);
  }
  auto rotate(const usize fn, const usize dim) const {
    return rotate_.at(dim).at(fn);
  }
  auto shuffle(const usize fn, const usize dim) const {
