...
inline auto apply_geom_transformations(std::span<const f64> input,
                                       problem_context_view ctx,
                                       affine_mask_t mask,
                                       f64 scale_mul = 1.0) {
  const auto nrow = input.size();
  auto y_out = std::vector<double>(nrow);
  auto z_out = std::vector<double>(nrow);
  sr_func(input, z_out, ctx.shift, ctx.rotate, scale_mul, mask.shift, mask.rot,
          y_out);
