...
void cec14_test_func(double *x, double *f, int nx, int mx, int func_num) {
    ...
    if (ini_flag == 0) {
        ...
        /* Load Matrix M*/
        sprintf(FileName, STORAGE_PREFIX("M_%d_D%d.txt").c_str(), func_num, nx);
        ...
        /* Load shift_data */
        sprintf(FileName, STORAGE_PREFIX("shift_data_%d.txt").c_str(), func_num);
        fpt = fopen(FileName, "r");
        if (fpt == NULL) {
            printf("\n Error: Cannot open input file for reading \n");
        }
        ...
        if (func_num >= 17 && func_num <= 22) {
            sprintf(FileName, STORAGE_PREFIX("shuffle_data_%d_D%d.txt").c_str(), func_num, nx);
            fpt = fopen(FileName, "r");
            if (fpt == NULL) {
                printf("\n Error: Cannot open input file for reading \n");
            }
