...
static void test_cec2017_equiv(void **state) {
  (void)state;
  ...
  for (int d = 0; d < 4; ++d) {
    for (int f = 1; f < fnNum + 1; ++f) {
      ...
      for (int i = 0; i < REPEATS; ++i) {
        ...
        cec2017_interface(dataPath, input, rewritten_output, dims[d], row, f);
        cec17_test_func(input, original_output, dims[d], row, f);
...
int main() {
  const struct CMUnitTest tests[] = {
    cmocka_unit_test(test_cec2017_equiv)
  };
