...
static int ini_flag=0,n_flag,func_flag;

static void Lennard_Jones(double *, int, double *); /* Lennard Jones */
static void Hilbert(double *, int, double *); /* Hilbert */
static void Chebyshev(double *, int, double *); /* Chebyshev */
...
{
	IS_NOT_USED int cf_num=10,i, IS_NOT_USED j;
	if (ini_flag==1)
	{
	...
	if (ini_flag==0)
	{
		...
		if (func_num > 3)
		{
		sprintf(FileName, "../data/cec2019/M_%d_D%d.txt", func_num,nx);
		...
		if (func_num > 3)
		{
		sprintf(FileName, "../data/cec2019/shift_data_%d.txt", func_num);
		fpt = fopen(FileName,"r");
		if (fpt==NULL)
		{
			printf("\n Error: Cannot open input file for reading \n");
...
IS_NOT_USED static void step_rastrigin_func (double *x, double *f, int nx, double *Os,double *Mr,int s_flag, int r_flag) /* Noncontinuous Rastrigin's  */
{
    int i;
	f[0]=0.0;
	for (i=0; i<nx; i++)
...
static void Lennard_Jones(double *x,int D, double *f)  // find the atomic configuration with minimum energy
{
...
IS_NOT_USED	static double minima[] = { -1.,-3.,-6.,-9.103852,-12.712062,-16.505384,-19.821489,-24.113360,
...
static void Hilbert(double *x, int D, double *f)  // find the inverse of the (ill-conditioned) Hilbert matrix
{
  ...
  double sum = 0;
	...
	static double hilbert[10][10], y[10][10];			// Increase matrix size if D > 100
...
static void Chebyshev( double *x, int D, double *f)  // Storn's Tchebychev - a 2nd ICEO function - generalized version
{
	/* Valid for any D>2
	constraints: unconstrained
	type: multi-modal with one global minimum; non-separable
