...
context_t::context_t(const cec_edition_t edition,
  ...
  std::ranges::for_each(dims.begin(), dims.end(), [&](const auto dim) {
    for (const auto &fn : std::views::iota(1, total_problem_num(edition) + 1)) {
      ...
      rotate_[dim].insert({fn_idx, load_table_data<table_type_t::rotate, f64>(
                                       datadir, dim, fn_idx)});
      shuffle_[dim].insert({fn_idx, load_table_data<table_type_t::shuffle, i64>(
                                        datadir, dim, fn_idx)});
      shift_[dim].insert({fn_idx, load_table_data<table_type_t::shift, f64>(
                                      datadir, dim, fn_idx)});
    }
  });
}
